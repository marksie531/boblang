...
public class BobLangPanel extends JPanel implements ActionListener,
	...
	private void loadGui(Element root2) {
		...
		JSplitPane pane = new JSplitPane(JSplitPane.VERTICAL_SPLIT,
				getControlPanel(), new JScrollPane(mainPanel));
	...
	private JSplitPane getControlPanel() {
		...
		JSplitPane left = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,
				getTitledScrolledPanel("Course", coursePanel, true),
				getTitledScrolledPanel("Lesson", lessonPanel, true));
		JSplitPane right = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,
				getTitledScrolledPanel("Topic", topicPanel, true),
				getTitledScrolledPanel("Actions", getActionsPanel(), false));
		JSplitPane both = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, left,
				right);
	 ...
	 * @return
	 */
	private JPanel getTitledScrolledPanel(String label, JPanel contentPanel,
			boolean buttons) {
		double[][] sizes = { { 5, FILL, 5, 35, 5, 45, 5 },
				{ 5, 20, 5, FILL, 5 } };
	...
	private void refreshCoursePanel() {
		...
		coursePanel.setLayout(new TableLayout(getTableLayoutSize(1,
				courseElms.size(), PREF, PREF)));
	...
	private void refreshLessonPanel() {
		...
		for (Element courseElm : courseElms) {
			...
			if (courseCB.isSelected()) {
				...
				for (int i = 0; i < lessonElms.size(); i++) {
					String lessonName = lessonElms.get(i).getAttributeValue(
							"name");
					JCheckBox lessonCB = new JCheckBox(lessonName);
					String lessonCBName = "lesson:" + lessonName + ",course:"
							+ courseName;
		...
		if (lessonCBS.size() > 0) {
			TableLayout layout = new TableLayout(getTableLayoutSize(1,
					lessonCBS.size(), FILL, PREF));
			...
			if (courseCB.isSelected()) {
				...
				for (Element lessonElm : lessonElms) {
					...
					if (lessonCB.isSelected()) {
						List<Element> topicElms = lessonElm
								.getChildren("topic");
	...
	private void examCheck() {
		...
		for (Component comp : mainPanel.getComponents()) {
			if (comp instanceof JTextField) {
				...
				if (answer.equalsIgnoreCase(input)) {
					...
					for (char c : REPLACES.toCharArray()) {
						rInput = input.replace(String.valueOf(c), "");
						rAnswer = answer.replace(String.valueOf(c), "");
					}
		...
		examLabel.setText(correct + " / " + total + " = " + (cp)
				+ "% correct, " + almost + " / " + total + " = " + (ap)
				+ "% almost");
	}
	...
	private void updateStates() {
		...
		for (Component comp : mainPanel.getComponents()) {
			if (comp instanceof JTextField
					&& ((JTextField) comp).getText().trim().equals("")) {
	 ...
	 * @return
	 */
	private double[][] getTableLayoutSize(int cols, int rows,
			double defaultCol, double defaultRow) {
		...
		if (event.getSource() instanceof JComponent) {
			...
			if (name != null) {
				if (name.equals("start")) {
					...
					String symbol = shift ? name.substring(7).toUpperCase()
							: name.substring(7);
					addSymbol(symbol);
				}
				updateStates();
	...
	private void select(String name, boolean state) {
		...
		if (name.indexOf("lesson") != -1)
			...
			comps = coursePanel.getComponents();
		...
		if ((e.getModifiers() & KeyEvent.CTRL_MASK) != 0) {
			try {
				if ((e.getKeyCode() >= 48 && e.getKeyCode() <= 57)
					...
					if ((e.getModifiers() & KeyEvent.SHIFT_MASK) != 0) {
						addSymbol(String.valueOf(specialChars.toUpperCase()
								.charAt(num)));
					} else {
